...
**验证方式**（根据功能类型选择）：
- ✅ **验证器**：执行npm命令验证（如 `npm run validate:state`）
- ✅ **命令**：手动执行命令验证
- ✅ **工具函数**：编写简单测试用例或手动调用
...
**更新内容**：
- 在对应专项指南中添加新功能说明
- 示例：验证器完成后，在 state-management.md 中添加验证器使用说明
...
```json
{
  ...
  "artifacts": ["CLAUDE.md", "[专项指南].md", "package.json"]
}
```
...
❌ 错误做法：
- 只更新一个位置
- 更新位置不合理（如命令功能只更新CLAUDE.md，不更新package.json）
```
