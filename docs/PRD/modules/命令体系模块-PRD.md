...
### 4.2 上下文加载集成 {#prd-命令体系-4.2}

> **说明**：上下文加载能力由**状态管理模块**提供（详见 PRD.md 4.2节、状态管理模块-PRD.md 3.5节）。命令体系模块作为调用方，在命令执行时调用上下文加载接口。
...
> **说明**：`/expand-context` 暂不实现。命令都是单独执行的，如需扩展上下文可通过 `/custom-task` 实现。

**`/custom-task` 命令格式**：

采用**用户显式指定**方案，不依赖AI理解能力推断上下文：

```bash
# 命令格式
/custom-task [--modules=模块1,模块2] "任务描述"

# 示例1：不指定模块，仅加载阶段级上下文
/custom-task "分析当前架构的性能瓶颈"
→ 加载当前阶段的基础上下文

# 示例2：指定模块，加载模块级上下文
/custom-task --modules=核心流程模型,状态管理模块 "分析两个模块的集成点"
→ 加载指定模块的上下文（PRD + 架构文档 + state.json相关字段）
```

**`/custom-task` 执行流程**：

```
用户: /custom-task --modules=核心流程模型,状态管理模块 "分析集成点"
        ↓
命令体系模块解析参数：
  - modules = ["核心流程模型", "状态管理模块"]
  - taskDescription = "分析集成点"
        ↓
对每个模块调用 getContextForModule(module, currentPhase)
        ↓
合并上下文，去重文件列表
        ↓
显示上下文列表，请求用户确认
...
验收标准：
  - [ ] 支持命令格式：/custom-task [--modules=模块1,模块2] "任务描述"
  - [ ] 不指定--modules时，仅加载当前阶段的基础上下文
  - [ ] 指定--modules时，加载指定模块的上下文（PRD + 架构 + state.json相关字段）
  - [ ] AI显示将加载的上下文列表，请求用户确认
  - [ ] 用户确认后，AI按照上下文执行任务
...
- **会话恢复**：每次新会话开始时读取state.json，了解当前阶段和模块进度，继续工作（注：不支持命令执行中途的断点恢复，因为会话中断时无法可靠捕获和保存状态）
