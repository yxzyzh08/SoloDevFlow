...
**核心目标**：确保AI在执行命令时专注于当前任务上下文，避免注意力分散导致任务质量下降。
...
每个命令文件必须定义以下四个维度的上下文：

| 维度 | 说明 | 必填 |
|------|------|------|
| **任务目标** | 本命令要完成什么 | 是 |
| **关注范围（Focus Scope）** | AI应主动读取的内容 | 是 |
| **注意力边界（Attention Boundary）** | AI不应主动读取的内容（除非必要） | 是 |
| **期望产出** | 命令执行后的交付物 | 是 |
...
| 命令 | 关注范围 | 注意力边界 |
|------|----------|------------|
| `/start-requirements` | state.json, PRD模板, 用户输入 | 架构文档, 代码, 测试 |
| `/start-architecture` | state.json, 已审批的PRD, 架构模板, 已完成的架构文档 | 代码, 测试, 历史迭代数据 |
| `/start-implementation` | state.json, 已审批的架构文档, 代码模板 | PRD（除非需要澄清）, 历史迭代 |
| `/start-testing` | state.json, PRD验收标准, 架构文档, 实现代码 | 历史迭代, 部署文档 |
| `/start-deployment` | state.json, 架构文档, 部署模板 | PRD, 代码实现细节 |

#### 4.2.4 逃逸机制

当预定义命令无法满足需求时，提供以下逃逸命令：

**`/custom-task <任务描述>`**：
- 用途：创建自定义任务
- 行为：AI根据任务描述自动推断关注范围，生成任务上下文供用户确认
- 使用场景：预定义命令无法覆盖的新场景

**`/expand-context <扩展内容描述>`**：
- 用途：临时扩展当前任务的上下文
- 行为：将指定内容添加到当前任务的关注范围
- 使用场景：当前任务需要读取注意力边界外的内容
- 约束：扩展仅对当前任务有效
