...
**ID格式**：`{文档类型}-{模块名}-{章节编号或名称}`

| 文档类型 | 前缀 | 示例 |
|----------|------|------|
| PRD（总） | `prd-project` | `prd-project-模块划分` |
| PRD（模块） | `prd-{模块名}` | `prd-状态管理-3.1` |
...
| **PRD（模块）** | 功能点清单/命令清单、数据模型、验收标准 | `{#prd-状态管理-3.1}`、`{#prd-命令体系-命令清单}` |
| **架构-系统总览** | 技术架构、部署架构、模块职责 | `{#arch-状态管理-技术架构}` |
| **架构-数据模型** | Schema定义、类型定义 | `{#arch-状态管理-schema}` |
| **架构-集成设计** | 接口定义、集成点 | `{#arch-状态管理-接口定义}` |
| **测试-E2E** | 用例清单、测试结果 | `{#test-E2E-用例清单}` |
| **测试-性能** | 测试指标、测试结果 | `{#test-性能-指标}` |
